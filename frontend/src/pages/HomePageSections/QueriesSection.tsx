import React from "react";
import '../../styles/HomePage.css'
import {Link} from "react-router-dom";

const QueriesSection: React.FC = () => {

    return (
        <div className="queries">
            <div className="content">
                <div className="top-text">
                    <h2>Explore and Develop Queries</h2>
                    <p>Using our IDE you can write long sentences that fill space and look cool if your a big enough nerd for this kinda stuff. <br /> Bet you think this is cool. Nerd.</p>
                </div>
                <div className="buttons">
                        <button className="translate">
                            <Link to="/translator" style={{ color: 'inherit', textDecoration: 'inherit'}}>Translate</Link>
                        </button>
                        <button className="documentation">
                            <Link to="/documentation" style={{ color: 'inherit', textDecoration: 'inherit'}}>Documentation</Link>
                        </button>
                </div>
                <div className="images">
                    <img src="https://via.placeholder.com/250" alt="IDE"/>
                </div>
            </div>
        </div>
    );
}

export default QueriesSection;